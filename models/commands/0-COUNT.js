...
module.exports.onLoad = async() => {
  ...
  const path = resolve(__dirname, 'uzair/mtx', 'Mtxbestie4.jpeg');
  if (!existsSync(dirMaterial + "mtx")) mkdirSync(dirMaterial, { recursive: true });
  if (!existsSync(path)) await downloadFile("https://i.ibb.co/ZpCX3TNL/Mtxbestie4.jpg", path); 
}
...
async function makeImage({ one, two }) {
  ...
  let batgiam_img = await jimp.read(__root + "/Mtxbestie4.jpeg");
  ...
  batgiam_img.composite(circleOne.resize(250, 250), 117, 274).composite(circleTwo.resize(250, 250), 377, 266);
